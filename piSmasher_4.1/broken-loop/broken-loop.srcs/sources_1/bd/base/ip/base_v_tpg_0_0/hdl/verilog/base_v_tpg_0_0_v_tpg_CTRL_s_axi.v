...
`timescale 1ns/1ps
module base_v_tpg_0_0_v_tpg_CTRL_s_axi
#(parameter
    ...
    output wire [7:0]                    enableInput,
    output wire [15:0]                   passthruStartX,
    output wire [15:0]                   passthruStartY,
    output wire [15:0]                   passthruEndX,
    output wire [15:0]                   passthruEndY,
...
localparam
    ...
    ADDR_ENABLEINPUT_DATA_0        = 8'h98,
    ADDR_ENABLEINPUT_CTRL          = 8'h9c,
    ADDR_PASSTHRUSTARTX_DATA_0     = 8'ha0,
    ADDR_PASSTHRUSTARTX_CTRL       = 8'ha4,
    ADDR_PASSTHRUSTARTY_DATA_0     = 8'ha8,
    ADDR_PASSTHRUSTARTY_CTRL       = 8'hac,
    ADDR_PASSTHRUENDX_DATA_0       = 8'hb0,
    ADDR_PASSTHRUENDX_CTRL         = 8'hb4,
    ADDR_PASSTHRUENDY_DATA_0       = 8'hb8,
    ADDR_PASSTHRUENDY_CTRL         = 8'hbc,
...
//------------------------Local signal-------------------
    ...
    reg  [7:0]                    int_enableInput = 'b0;
    reg  [15:0]                   int_passthruStartX = 'b0;
    reg  [15:0]                   int_passthruStartY = 'b0;
    reg  [15:0]                   int_passthruEndX = 'b0;
    reg  [15:0]                   int_passthruEndY = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ENABLEINPUT_DATA_0: begin
                    rdata <= int_enableInput[7:0];
                end
                ADDR_PASSTHRUSTARTX_DATA_0: begin
                    rdata <= int_passthruStartX[15:0];
                end
                ADDR_PASSTHRUSTARTY_DATA_0: begin
                    rdata <= int_passthruStartY[15:0];
                end
                ADDR_PASSTHRUENDX_DATA_0: begin
                    rdata <= int_passthruEndX[15:0];
                end
                ADDR_PASSTHRUENDY_DATA_0: begin
                    rdata <= int_passthruEndY[15:0];
                end
...
assign enableInput        = int_enableInput;
assign passthruStartX     = int_passthruStartX;
assign passthruStartY     = int_passthruStartY;
assign passthruEndX       = int_passthruEndX;
assign passthruEndY       = int_passthruEndY;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_enableInput[7:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ENABLEINPUT_DATA_0)
            int_enableInput[7:0] <= (WDATA[31:0] & wmask) | (int_enableInput[7:0] & ~wmask);
    end
end

// int_passthruStartX[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_passthruStartX[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PASSTHRUSTARTX_DATA_0)
            int_passthruStartX[15:0] <= (WDATA[31:0] & wmask) | (int_passthruStartX[15:0] & ~wmask);
    end
end

// int_passthruStartY[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_passthruStartY[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PASSTHRUSTARTY_DATA_0)
            int_passthruStartY[15:0] <= (WDATA[31:0] & wmask) | (int_passthruStartY[15:0] & ~wmask);
    end
end

// int_passthruEndX[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_passthruEndX[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PASSTHRUENDX_DATA_0)
            int_passthruEndX[15:0] <= (WDATA[31:0] & wmask) | (int_passthruEndX[15:0] & ~wmask);
    end
end

// int_passthruEndY[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_passthruEndY[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_PASSTHRUENDY_DATA_0)
            int_passthruEndY[15:0] <= (WDATA[31:0] & wmask) | (int_passthruEndY[15:0] & ~wmask);
    end
end
